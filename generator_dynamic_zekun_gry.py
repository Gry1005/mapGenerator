...
def text_on_canvas(text, myf, ro, color=(0.5, 0.5, 0.5), margin=1):
    ...
    #！！！！
    color0=round(color[0],4)
    color1 = round(color[1], 4)
    color2 = round(color[2], 4)
    #plt.title(myf.get_name()+' color:'+str(color0)+","+str(color1)+","+str(color2)+' fontsize:'+myf.get_size())
    plt.savefig("../dynamic_write_txt/"+str(myf.get_name()).replace(" ","-")+'-color:'+str(color0)+","+str(color1)+","+str(color2)++"-fontsize:"+str(myf.get_size())+".png",dpi=400,bbox_inches='tight')
...
class SynthMap_DataGenerator_Centerline_Localheight_Dynamic(object):
    ...
    def __init__(self, image_root_path, fonts_path, GB_path, batch_size=128, seed=1234, mode='training',
                 border_percent=0.15, overlap=True):
    ...
    def __getitem__(self, batch_idx):
        ...
        for i, patch_path in enumerate(subset_X):
            ...
            while (text_num < text_num_thresh):
                ...
                text = word_set[np.random.randint(1, words_size - 1)].strip()
                ...
                # font specification
                font_face = fonts[np.random.randint(0, len(fonts) - 1)]

                font_size = np.random.randint(10, 80)
                ...
                ro = np.random.randint(-90, 90)
                fcolor = np.random.randint(0, 128) * np.ones((3))
                ...
                if text_layer != 0:
                    ...
                    while True:
                        breakflag = True
                        upper_left_pos = (
                            np.random.randint(0, W - text_w / 2),
                            np.random.randint(0, H - text_h / 2))  # upper-left pos on bg img
