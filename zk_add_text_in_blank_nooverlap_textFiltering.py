...
#big font, single char

#map4_path = 'E:/Spatial Computing & Informatics Laboratory/CutTextArea/dataset4/backGround4/*/'
map4_path = 'E:/Spatial Computing & Informatics Laboratory/CutTextArea/dataset/concat_out/'
save_path = 'D:/pycharmWorkspace/mapGenerator/synthMap_application/concat_out_bigF_sChar/'
fonts_path = 'D:/pycharmWorkspace/mapGenerator/synthMap_application/generate_data/fonts/'

text_path='D:/pycharmWorkspace/mapGenerator/synthMap_application/generate_data/GB.txt'
...
for line in geoname_f:
    ...
    for w in words:
        ...
        for i in range(0, len(w)):
            if not (('a' <= w[i] <= 'z') or ('A' <= w[i] <= 'Z') or ('0' <= w[i] <= '9')):
                wFlag = False
                break
...
for i, patch_path in enumerate(map4_set):
    ...
    #i=random.randint(0,len(map4_set)-1)
    #patch_path=map4_set[i]

    print('processing', patch_path)
    img_bg_pil = Image.open(patch_path).convert('RGBA')

    #white background
    #gray0 = np.zeros((512, 512), dtype=np.uint8)
    #gray0[:, :] = 255
    #gray255 = gray0[:, :]

    #img_bg_pil=cv2.cvtColor(gray255, cv2.COLOR_GRAY2RGB)
    #img_bg_pil[:, :, 0:3] = 255

    #img_bg_pil = Image.fromarray(cv2.cvtColor(img_bg_pil, cv2.COLOR_BGR2RGB))

    # resize到512*512
    img_bg_pil = img_bg_pil.resize((512, 512), Image.ANTIALIAS)

    W, H = img_bg_pil.size
    #print(W,H)

    img_bg_cv = cv2.imread(patch_path)
    with open(save_path + str(cnt)+'_'+ os.path.basename(patch_path)[:-4] + '.txt', 'w') as f:
        1
    info_f = open(save_path + str(cnt)+'_' + os.path.basename(patch_path)[:-4] + '.txt', 'w')
    ...
    while (text_num < text_num_thresh):
        ...
        if not np.random.randint(0, 4):
            text = chr(np.random.randint(97, 123))

        # font specification
        ind=random.randint(0, len(fonts) - 1)
        font_face = fonts[ind]
        ...
        # 更大字体
        if not np.random.randint(0, 4):
            font_size = random.randint(120, 150)
        ...
        if text_layer != 0:
            ...
            while True:
                breakflag = True
                upper_left_pos = (
                random.randint(0, W - text_w / 2), random.randint(0, H - text_h / 2))  # upper-left pos on bg img
                ...
                if len(text_region) == 0:
                    ...
                    and_area = np.sum(
                        np.float32(np.greater(masked_and, 10)))  # use and_are to check if masked_and has overlap area
    ...
    img_bg_pil.convert('RGB').save(save_path + str(cnt)+'_' +os.path.basename(patch_path))
    #plt.imshow(img_bg_pil.convert('RGB'))

    #print(type(img_bg_pil.convert('RGB')))
